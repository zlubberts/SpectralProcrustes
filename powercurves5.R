#Experiment 5: Salt-and-pepper noise,
#             measure of strength of HA is given by sigma = proportion of 
#             resampled rows of X
...
perturb <- function(X,Y,perm,sigma,n){
  #X and Y are both drawn using getX
  #perm <- sample(1:n) where n=number of rows of X and Y
  #sigma = proportion of rows to be replaced
  x <- 1:n
  idxs <- sort(perm[1:floor(sigma*n)])
  nidxs <- x[-idxs]
  return(diag((x%in%idxs)*1)%*%Y+diag((x%in%nidxs)*1)%*%X)
}
...
#save(powercurves,file="powercurves5.RData")
